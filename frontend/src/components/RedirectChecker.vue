<template>
  <div class="tool-page">
    <h2 class="text-3xl font-bold mb-4">Redirect Checker</h2>
    <p class="text-lg mb-6">
      <strong>Description</strong>: The Redirect Checker tool helps you check the redirection status of a given URL.
    </p>
...
https://www.example.com
        ...
        <p class="text-lg mb-6">Enter a URL to check its redirection status.</p>
      </div>
    </div>
    <div class="tool-form">
      <form @submit.prevent="checkRedirect">
        ...
        <div class="tool-actions">
          <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" :disabled="loading">
            ...
            <span v-else>Check Redirect</span>
          </button>
        </div>
      </form>
    </div>
    <div v-if="result || error" class="generated-info mt-4">
      <h3 class="text-2xl font-bold mb-2">Redirect Status</h3>
      <div v-if="result" class="result">
        <p><strong>Status Code:</strong> {{ result.statusCode }}</p>
        <p><strong>Final URL:</strong> <a :href="result.finalUrl" target="_blank" class="text-blue-500 hover:text-blue-700">{{ result.finalUrl }}</a></p>
      </div>
