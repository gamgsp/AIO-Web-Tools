<template>
  <div class="tool-page">
    <h2 class="text-3xl font-bold mb-4">HTML Formatter</h2>
    <p class="text-lg mb-6">
      <strong>Description</strong>: The HTML Formatter tool helps you format and beautify your HTML code.
    </p>
...
&lt;/div&gt;
        ...
        <p class="text-lg mb-6">Click "Beautify" to format the HTML code.</p>
      </div>
    </div>
    <div class="tool-form">
      <form @submit.prevent="processHtml">
        ...
        <div class="tool-actions">
          <button type="button" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" @click="beautifyHtml" :disabled="loading">
            Beautify
          </button>
        </div>
      </form>
    ...
    <div v-if="result || error" class="generated-info mt-4">
      <h3 class="text-2xl font-bold mb-2">Formatted HTML</h3>
...
export default {
  ...
  methods: {
    beautifyHtml() {
      ...
      this.error = '';
      try {
        this.result = beautifyHtml(this.htmlCode);
      } catch (error) {
